package com.cb.itoken.web.backend.Interceptor;
...
import com.cb.itoken.web.backend.service.RedisService;
...
public class WebBackendInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) {
        ...
        if(StringUtils.isBlank(token)){
            try {
                response.sendRedirect(String.format("%s/login?url=http://localhost:8603/%s", hosts_sso, request.getServletPath()));
            } catch (IOException e) {
                e.printStackTrace();
            }
            return false;
